...
|------|--------------------------------------------------------------|
| Read a type's template + requirements before writing | `deliverables get ID --include meta,template,requirements,writer_instructions` |
| Find the `--type` id for a new deliverable | `deliverables types` |
| **Create a deliverable** | `deliverables create --project ID --type TYPE_ID --title "…" --content @file` |
| Edit content (autosave, no version bump) | `deliverables edit ID --content @file` |
| Rename | `deliverables rename ID --title "…"` |
| Snapshot a restorable version | `deliverables save-version ID --reason "…"` |
| Change state | `deliverables set-state ID --state "Review"` |
| Create → fill → snapshot in one atomic call | `deliverables write --ops -` |
...
MCP `expedait:write_deliverable` tool). Chain on a fresh deliverable with `id: "$last"`, or bind
...
the CLI, the same workflow maps to one tool, `expedait:write_deliverable(ops=[...])`, with the
identical op set (`create` / `edit` / `rename` / `save_version` / `set_state`) and `$last` /
`@ref` chaining. Reads map to `expedait:get_deliverable(id, include=[...])`,
`expedait:get_deliverable_context(id)`, and `expedait:get_objective_overview(id)`. Tool names
are `ServerName:tool` — the server is `expedait`. Requires the `mcp:deliverables:write` scope —
if those tools aren't in your tool list, the connector isn't attached; use the CLI above instead.
